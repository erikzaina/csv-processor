...
public class CsvService {
    ...
    public List<CsvInfo> sortByNome(List<CsvInfo> csvList) {
        csvList.sort(Comparator.comparing(line -> line.getNome(), String.CASE_INSENSITIVE_ORDER));
        return csvList;
    }
    ...
    private void validateCsvFile(MultipartFile file) {
        ...
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(file.getInputStream(), StandardCharsets.UTF_8))) {
            ...
            String normalizedActual = actualHeader.replaceAll("[^\\p{Print}]", "").replaceAll("\\s+", "").toLowerCase();
            String normalizedExpected = EXPECTED_HEADER.replaceAll("[^\\p{Print}]", "").replaceAll("\\s+", "").toLowerCase();
